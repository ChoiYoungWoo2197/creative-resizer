...
ARTIFACT_DIR="artifacts/stage8-smoke/${TIMESTAMP}"
...
# ── Step 1: 아티팩트 디렉토리 생성 ───────────────────────────────────────────
section "1: Create artifact directory"
mkdir -p "$ARTIFACT_DIR"
echo "Stage 8 Smoke Test started: ${TIMESTAMP}" > "$SMOKE_LOG"
echo "Project: ${PROJECT_NAME}" >> "$SMOKE_LOG"
echo "Host: $(hostname)" >> "$SMOKE_LOG"
success "Artifact dir: ${ARTIFACT_DIR}"

# ── Step 2: git 상태 스냅샷 ──────────────────────────────────────────────────
section "2: Git snapshot"
git rev-parse HEAD > "${ARTIFACT_DIR}/git_commit.txt" 2>&1 || echo "(no git)" > "${ARTIFACT_DIR}/git_commit.txt"
...
# Step 26: cleanup trap
# 비정상 종료 코드는 SMOKE_RUNNER_EXIT에 저장됨 → trap에서 exit $SMOKE_RUNNER_EXIT 호출
# trap cleanup EXIT → cleanup() 마지막에 exit $code
